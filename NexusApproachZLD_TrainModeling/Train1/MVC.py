...
def build():
    ...
    m.fs.properties_feed.set_default_scaling(
        "flow_mass_phase_comp", 1e0, index=("Liq", "TDS")
    )
    ...
    # unit model values -- distillate, brine, and/or evaporator scaling may need adjusting based on MVC recovery changes
    # pumps
    ...
    # distillate HX    
    iscale.set_scaling_factor(m.fs.hx_distillate.hot.heat, 1e-4)
    iscale.set_scaling_factor(m.fs.hx_distillate.cold.heat, 1e-4)
    ...
    # brine HX 
    iscale.set_scaling_factor(m.fs.hx_brine.hot.heat, 1e-4)
    iscale.set_scaling_factor(m.fs.hx_brine.cold.heat, 1e-4)
...
def set_operating_conditions(m):
    # Feed inlet
    m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "TDS"].fix(37.96)
    m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "H2O"].fix(436.35)
    m.fs.feed.properties[0].temperature.fix(273.15 + 25)
    m.fs.feed.properties[0].pressure.fix(101325)

    m.fs.recovery[0].fix(0.4) #vary for different recovery scenarios
    ...
    m.fs.hx_distillate.area.fix(700)
    ...
    m.fs.evaporator.area.setub(5e4)  # m^2
...
def display_metrics(m):
    print("\nSystem metrics")
    print(
        "Feed flow (vol):                          %.2f m3/s"
        % value(m.fs.feed.properties[0].flow_vol)
    )
    print(
        "Distillate flow (vol):                    %.2f m3/s"
        % value(m.fs.distillate.properties[0].flow_vol)
    )
    print(
        "Brine flow (vol):                         %.2f m3/s"
        % value(m.fs.brine.properties[0].flow_vol)
    )
    print(
        "Volumetric yield:                         %.2f %%vol"
        % (
            value(m.fs.distillate.properties[0].flow_vol/m.fs.feed.properties[0].flow_vol*100)
        )
    )
    print(
