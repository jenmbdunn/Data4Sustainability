...
from watertap.examples.flowsheets.lsrro import lsrro_basecode_Dhe


def _lsrro_presweep(
    number_of_stages=3, A_value=4.2e-12, permeate_quality_limit=0.0007, has_CP=True, 
    Cin=65.223, Qin=0.2846,
):
    m = lsrro_basecode_Dhe.build(
        number_of_stages=number_of_stages,
        has_NaCl_solubility_limit=False,
        has_calculated_concentration_polarization=has_CP,
        has_calculated_ro_pressure_drop=True, 
    )
    lsrro_basecode_Dhe.set_operating_conditions(m, Cin, Qin)
    lsrro_basecode_Dhe.initialize(m)
    lsrro_basecode_Dhe.solve(m)
    lsrro_basecode_Dhe.optimize_set_up(
        m,
...
def run_case(number_of_stages, nx, output_filename=None):
    """
    Run the parameter sweep tool on the LSRRO flowsheet, sweeping over recoveries.
    Arguments
    ---------
    number_of_stages (int) : The number of LSRRO Stages (including the initial RO stage).
    nx (int) : The number of points for water recovery. The
    ...
    sweep_params["Volumetric Recovery Rate"] = LinearSample(
        m.fs.water_recovery, 0.521, 0.732, nx
    )
    ...
    outputs["Final Brine Concentration (ppm)"] = (m.fs.disposal.properties[
        0
    ].mass_frac_phase_comp["Liq", "TDS"] * 1e6)
    outputs["Final Brine Mass Flow (kg/s)"] = (m.fs.disposal.properties[0].flow_mass_phase_comp["Liq", "TDS"]
                                              + m.fs.disposal.properties[0].flow_mass_phase_comp["Liq", "H2O"])
    outputs["Final Permeate Concentration (ppm)"] = (
        m.fs.product.properties[0].mass_frac_phase_comp["Liq", "TDS"] * 1e6
    )
    outputs["Annual Feed Flow"] = m.fs.feed.properties[0].flow_vol * 3.154e7
    outputs["Annual Water Production"] = m.fs.product.properties[0].flow_vol * 3.154e7
    ...
    outputs["Total Capex LCOW"] = (
        ...
        * m.fs.costing.factor_capital_annualization
        / (m.fs.product.properties[0].flow_vol * 3.154e7)
    )
    outputs["Total Opex LCOW"] = (
        m.fs.costing.total_operating_cost / (m.fs.product.properties[0].flow_vol * 3.154e7) #/ m.fs.costing.annual_water_production
    )
    ...
    outputs.update(
        {
            f"Observed Salt Passage (%)-Stage {idx}": (
                1 - stage.rejection_phase_comp[0, "Liq", "TDS"]
            )
            * 100
            for idx, stage in m.fs.ROUnits.items()
        }
    ...
    outputs.update(
        {
            f"Mass Salt Passage (%)-Stage {idx}": (
                stage.mixed_permeate[0].flow_mass_phase_comp["Liq", "TDS"]
                / stage.feed_side.properties[0, 0].flow_mass_phase_comp["Liq", "TDS"]
            )
            * 100
            for idx, stage in m.fs.ROUnits.items()
        }
    ...
    outputs.update(
        {
            f"B-Value (LMH)-Stage {idx}": stage.B_comp[0, "TDS"] * 3.6e6
            for idx, stage in m.fs.ROUnits.items()
        }
    )
    ...
    outputs.update(
        {
            f"Average TDS Flux (GMH)-Stage {idx}": stage.flux_mass_phase_comp_avg[
                0, "Liq", "TDS"
            ]
            * 3.6e6
            for idx, stage in m.fs.ROUnits.items()
    ...
    global_results = parameter_sweep(
        ...
        optimize_function=lsrro_basecode_Dhe.solve,
...
if __name__ == "__main__":
    global_results, sweep_params, m = run_case(number_of_stages=3, nx=20,)
