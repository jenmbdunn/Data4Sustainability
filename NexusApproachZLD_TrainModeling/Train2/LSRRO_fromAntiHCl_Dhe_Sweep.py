...
def run_case(number_of_stages, nx, output_filename=None):
    """
    Run the parameter sweep tool on the LSRRO flowsheet, sweeping over specified recoveries.
    Arguments
    ---------
    ...
    sweep_params["Volumetric Recovery Rate"] = LinearSample(
        m.fs.water_recovery, 0.395, 0.689, nx
    )
    ...
    outputs["Total Opex LCOW"] = (
        m.fs.costing.total_operating_cost / (m.fs.product.properties[0].flow_vol * 3.154e7)
    )
